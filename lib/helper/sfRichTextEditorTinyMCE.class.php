...
tinyMCE.init({
  ...
  '.(isset($options['tinymce_options']) ? ','.$options['tinymce_options'] : '').'
        ...
        return
          content_tag('script', javascript_cdata_section($tinymce_js), ['type' => 'text/javascript']).
          content_tag('textarea', $this->content, array_merge(['name' => $this->name, 'id' => $id], _convert_options($options)));
    }
}
