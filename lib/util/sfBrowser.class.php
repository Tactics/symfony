...
class sfBrowser
{
    ...
    public function call($uri, $method = 'get', $parameters = [], $changeStack = true)
    {
        ...
        [$path, $query_string] = false !== ($pos = strpos((string) $uri, '?')) ? [substr((string) $uri, 0, $pos), substr((string) $uri, $pos + 1)] : [$uri, ''];
        ...
        $_SERVER['HTTP_HOST'] = $this->hostname ?: sfConfig::get('sf_app').'-'.sfConfig::get('sf_environment');
        $_SERVER['SERVER_NAME'] = $_SERVER['HTTP_HOST'];
        $_SERVER['SERVER_PORT'] = 80;
        $_SERVER['HTTP_USER_AGENT'] = 'PHP5/CLI';
        $_SERVER['REMOTE_ADDR'] = $this->remote ?: '127.0.0.1';
        ...
        if (preg_match('/(x|ht)ml/i', (string) $response->getContentType(), $matches)) {
    ...
    {
        if (false !== $pos = strpos((string) $name, '[')) {
            $var = &$vars;
            $tmps = array_filter(preg_split('/(\[ | \[\] | \])/x', (string) $name), fn ($s) => $s !== '');
            foreach ($tmps as $tmp) {
                $var = &$var[$tmp];
            }
            if ($var) {
    ...
    protected function fixUri($uri)
    {
        ...
        if (str_starts_with((string) $uri, 'http')) {
            ...
            $uri = substr((string) $uri, strpos((string) $uri, 'index.php') + strlen('index.php'));
        }
