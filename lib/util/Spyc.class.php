...
class Spyc
{
            ...
            if ($this->_inBlock === false && empty($ifchk)) {
                ...
                if ($this->_lastIndent == $node->indent) {
                    ...
                    if ($this->_inBlock === true) {
                        ...
                        if ($this->_blockEnd = "\n") {
                            $last = &$this->_allNodes[$this->_lastNode];
                            $last->data[key($last->data)] = php7_trim((string) $last->data[key($last->data)]);
                        }
                    }
                ...
                if ($this->_inBlock === false) {
                    ...
                    if (
                        ...
                        foreach ($node->data[key($node->data)] as $d) {
                            if (!is_array($d) && (preg_match('/^&([^ ]+)/', (string) $d) || preg_match('/^\*([^ ]+)/', (string) $d))) {
                                $this->_haveRefs[] = &$this->_allNodes[$node->id];
                            }
                        }
                    }
    ...
    protected function _toType($value)
    {
        ...
        if (preg_match('/^("(.*)"|\'(.*)\')/', (string) $value, $matches)) {
        ...
        } elseif (preg_match('/^\\[\s*\\]$/', (string) $value, $matches)) {
            $value = [];
        ...
        } elseif (str_contains((string) $value, ': ') && !preg_match('/^{(.+)/', (string) $value)) {
            // It's a map
            ...
            $value = $array;
        } elseif (strtolower((string) $value) == 'null' or $value == '' or $value == '~') {
            $value = null;
        } elseif (ctype_digit((string) $value)) {
            $value = (int) $value;
        } elseif (in_array(strtolower((string) $value), ['true', 'on', '+', 'yes', 'y'])) {
            $value = true;
        } elseif (in_array(strtolower((string) $value), ['false', 'off', '-', 'no', 'n'])) {
            $value = false;
    ...
    public function _linkRef(&$n, $key, $k = null, $v = null)
    {
        if (empty($k) && empty($v)) {
            ...
            if (preg_match('/^&([^ ]+)/', (string) $n->data[$key], $matches)) {
                ...
                $this->_allNodes[$n->id]->data[$key] = substr((string) $n->data[$key], strlen($matches[0]) + 1);
            // Look for *refs
            ...
            if (preg_match('/^&([^ ]+)/', (string) $v, $matches)) {
                // Flag the node so we know it's a reference
                $this->_allNodes[$n->id]->ref = substr($matches[0], 1);
                $this->_allNodes[$n->id]->data[$key][$k] = substr((string) $v, strlen($matches[0]) + 1);
            // Look for *refs
