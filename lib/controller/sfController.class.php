...
abstract class sfController
{
     ...
     * @param bool Whether to throw exceptions if the controller doesn't exist
     *
    ...
    protected function controllerExists($moduleName, $controllerName, $extension, $throwExceptions)
    {
        ...
        foreach ($dirs as $dir => $checkEnabled) {
            ...
            if (is_readable($module_file)) {
                ...
                if (!in_array('execute'.ucfirst((string) $controllerName), get_class_methods($moduleName.$classSuffix.'s'))) {
                    if ($throwExceptions) {
                        throw new sfControllerException(sprintf('There is no "%s" method in your action class "%s"', 'execute'.ucfirst((string) $controllerName), $moduleName.$classSuffix.'s'));
                    }
    ...
    public function forward($moduleName, $actionName)
    {
        ...
        $moduleName = $moduleName ? preg_replace('/[^a-z0-9\-_]+/i', '', $moduleName) : $moduleName;
        $actionName = $actionName ? preg_replace('/[^a-z0-9\-_]+/i', '', $actionName) : $actionName;
        ...
        if ($this->getActionStack()->getSize() == 1 && sfConfig::get('mod_'.strtolower((string) $moduleName).'_is_internal') && !sfConfig::get('sf_test')) {
    ...
    public function getView($moduleName, $actionName, $viewName)
    {
        ...
        if (is_readable($file)) {
            ...
            $viewName = $this->getContext()->getRequest()->getAttribute($moduleName.'_'.$actionName.'_view_name', sfConfig::get('mod_'.strtolower((string) $moduleName).'_view_class'), 'symfony/action/view');
            $class = $viewName.'View';
        }
