...
class sfRouting
{
     ...
     *
     * @param bool Whether to give an internal URI with the route name (@route)
    ...
    public function getCurrentInternalUri($with_route_name = false)
    {
        if ($this->current_route_name) {
            [$url, $regexp, $names, $names_hash, $defaults, $requirements, $suffix] = $this->routes[$this->current_route_name];
            ...
            if ($with_route_name) {
                ...
                $internal_uri = $request->getParameter('module', $defaults['module'] ?? '').'/'.$request->getParameter('action', $defaults['action'] ?? '');
            }
            ...
            foreach ($names as $name) {
                ...
                $params[] = $name.'='.$request->getParameter($name, $defaults[$name] ?? '');
            }
     ...
     * @see connect
     */
    public function prependRoute($name, $route, $default = [], $requirements = [])
    {
        $routes = $this->routes;
        $this->routes = [];
     ...
     * @see connect
     */
    public function appendRoute($name, $route, $default = [], $requirements = [])
    {
     ...
     */
    public function connect($name, $route, $default = [], $requirements = [])
    {
        ...
        if (($route == '') || ($route == '/')) {
            $regexp = '/^[\/]*$/';
            $this->routes[$name] = [$route, $regexp, [], [], $default, $requirements, $suffix];
        } else {
            $elements = [];
            ...
            } elseif ($route[strlen((string) $route) - 1] == '/') {
                $suffix = '/';
            }

            $regexp_suffix = preg_quote((string) $suffix);
            ...
            foreach ($elements as $element) {
                if (preg_match('/^:(.+)$/', $element, $r)) {
                    ...
                    if (isset($requirements[$element])) {
                        ...
                        if (str_starts_with((string) $regex, '^')) {
                            $regex = substr((string) $regex, 1);
                        }
                        if (strlen((string) $regex) - 1 === strpos((string) $regex, '$')) {
            ...
            $this->routes[$name] = [$route, $regexp, $names, $names_hash, $default, $requirements, $suffix];
        }
    ...
    public function generate($name, $params, $querydiv = '/', $divider = '/', $equals = '/')
    {
        ...
        if ($name) {
            ...
            [$url, $regexp, $names, $names_hash, $defaults, $requirements, $suffix] = $this->routes[$name];
            ...
            foreach ($this->routes as $name => $route) {
                [$url, $regexp, $names, $names_hash, $defaults, $requirements, $suffix] = $route;
                if ($global_defaults !== null) {
                    $defaults = array_merge($defaults, $global_defaults);
                }
                ...
                foreach ($requirements as $req_param => $req_regexp) {
                    if (!preg_match('/'.str_replace('/', '\\/', $req_regexp).'/', (string) $tparams[$req_param])) {
                        continue 2;
                    }
                }
        ...
        $real_url = preg_replace_callback(
            '/\:([^\/]+)/',
            fn ($matches) => urlencode((string) $params[$matches[1]]),
            $url
        );
        ...
        if (strpos($real_url, '*')) {
            ...
            foreach ($params as $key => $value) {
                ...
                if (is_array($value)) {
                    foreach ($value as $v) {
                        $tmp[] = $key.$equals.urlencode((string) $v);
                    }
                } else {
                    $tmp[] = urlencode((string) $key).$equals.urlencode((string) $value);
                }
            }
    ...
    public function parse($url)
    {
        ...
        foreach ($this->routes as $route_name => $route) {
            ...
            [$route, $regexp, $names, $names_hash, $defaults, $requirements, $suffix] = $route;
            ...
            if (preg_match($regexp, $url, $r)) {
                ...
                foreach ($defaults as $name => $value) {
                    if (preg_match('#[a-z_\-]#i', (string) $name)) {
                        $out[$name] = urldecode((string) $value);
                    } else {
                        $out[$value] = true;
                    }
                }
