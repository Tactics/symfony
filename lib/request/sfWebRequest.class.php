...
class sfWebRequest extends sfRequest
{
    ...
    public function getFileValues($name)
    {
        if (strpos((string) $name, '[')) {
            ...
            return $this->filesInfos[$name] ?? null;
        }
    }
    ...
    public function initialize($context, $parameters = [], $attributes = [])
    {
        ...
        if (isset($_SERVER['REQUEST_METHOD'])) {
            match ($_SERVER['REQUEST_METHOD']) {
                'GET' => $this->setMethod(self::GET),
                'POST' => $this->setMethod(self::POST),
                'PUT' => $this->setMethod(self::PUT),
                'DELETE' => $this->setMethod(self::DELETE),
                'HEAD' => $this->setMethod(self::HEAD),
                default => $this->setMethod(self::GET),
            };
        } else {
    ...
    protected function getPathInfoArray()
    {
        if (!$this->pathInfoArray) {
            // parse PATH_INFO
            match (sfConfig::get('sf_path_info_array')) {
    ...
    public function isAbsUri()
    {
        ...
        return isset($pathArray['REQUEST_URI']) ? preg_match('/^http/', (string) $pathArray['REQUEST_URI']) : false;
    }
    ...
    public function getPathInfo()
    {
        ...
        if (!isset($pathArray[$sf_path_info_key]) || !$pathArray[$sf_path_info_key]) {
            if (isset($pathArray['REQUEST_URI'])) {
                ...
                $pathInfo = $pathArray['REQUEST_URI'] ? preg_replace('/^'.preg_quote($uri_prefix, '/').'/', '', $pathArray['REQUEST_URI']) : $pathArray['REQUEST_URI'];
                $pathInfo = $pathInfo ? preg_replace('/^'.preg_quote($script_name, '/').'/', '', $pathInfo) : $pathInfo;
                $prefix_name = $script_name ? preg_replace('#/[^/]+$#', '', $script_name) : $script_name;
                $pathInfo = $pathInfo ? preg_replace('/^'.preg_quote((string) $prefix_name, '/').'/', '', $pathInfo) : $pathInfo;
                $pathInfo = $pathInfo ? preg_replace('/\??'.preg_quote((string) $pathArray['QUERY_STRING'], '/').'$/', '', $pathInfo) : $pathInfo;
            }
        } else {
            $pathInfo = $pathArray[$sf_path_info_key];
            if ($sf_relative_url_root = $this->getRelativeUrlRoot()) {
                $pathInfo = $pathInfo ? preg_replace('/^'.str_replace('/', '\\/', $sf_relative_url_root).'\//', '', $pathInfo) : $pathInfo;
            }
        }
    ...
    public function getHost()
    {
        ...
        return $pathArray['HTTP_X_FORWARDED_HOST'] ?? $pathArray['HTTP_HOST'] ?? '';
    }
    ...
    public function getMethodName()
    {
        ...
        return $pathArray['REQUEST_METHOD'] ?? 'GET';
    }
    ...
    {
        if ($prefix) {
            $prefix = strtoupper((string) $prefix).'_';
        }
    ...
    public function isSecure()
    {
        ...
        return
          (isset($pathArray['HTTPS']) && (strtolower((string) $pathArray['HTTPS']) == 'on' || strtolower((string) $pathArray['HTTPS']) == 1))
          || (isset($pathArray['HTTP_X_FORWARDED_PROTO']) && strtolower((string) $pathArray['HTTP_X_FORWARDED_PROTO']) == 'https')
        ;
    }
    ...
    public function getRelativeUrlRoot()
    {
        if ($this->relativeUrlRoot === null) {
            ...
            $preg = $name ? preg_replace('#/[^/]+\.php5?$#', '', $this->getScriptName()) : $name;
            $this->relativeUrlRoot = sfConfig::get('sf_relative_url_root', $preg);
        }
     ...
     */
    protected function arrayToPaths($array = [], $prefix = '')
    {
        $str = '';
        $freshPrefix = $prefix;
