    ...
    public function log($message, $priority, $priorityName)
    {
        ...
        if (function_exists('xdebug_get_function_stack') && !isset($_GET['XDEBUG_SESSION_START']) && !isset($_COOKIE['XDEBUG_SESSION'])) {
            foreach (xdebug_get_function_stack() as $i => $stack) {
                if (
                    (isset($stack['function']) && !in_array($stack['function'], ['emerg', 'alert', 'crit', 'err', 'warning', 'notice', 'info', 'debug', 'log']))
        ...
        // get log type in {}
        $type = 'sfOther';
        if (preg_match('/^\s*{([^}]+)}\s*(.+?)$/', (string) $message, $matches)) {
            $type = $matches[1];
            $message = $matches[2];
        }

        // build the object containing the complete log information.
        $logEntry = ['priority' => $priority, 'priorityString' => $priorityName, 'time' => time(), 'message' => $message, 'type' => $type, 'debugStack' => $debug_stack];
