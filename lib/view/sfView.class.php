...
abstract class sfView
{
     ...
     */
    public const HEADER_ONLY = 'Headers';
    ...
    {
        return $this->escaping ?? sfConfig::get('sf_escaping_strategy');
    }
    ...
    {
        $method = $this->escapingMethod ?? sfConfig::get('sf_escaping_method');
    ...
    public function importAttributes($names, $files = false, $errors = true, $stripTags = true, $specialChars = true)
    {
        ...
        foreach ($names as &$name) {
            if (preg_match('/^([a-z0-9\-_]+)\{([a-z0-9\s\-_]+)\}$/i', (string) $name, $match)) {
                // we have a parent
                $parent = $match[1];
                $subname = $match[2];
    ...
    public function initialize($context, $moduleName, $actionName, $viewName)
    {
        ...
        $this->parameterHolder->add(sfConfig::get('mod_'.strtolower((string) $moduleName).'_view_param', []));
     ...
     *
     * @param bool Set the decorating mode for the view
     */
     ...
     */
    abstract public function render($templateVars = null);
    ...
    public function setDecoratorTemplate($template)
    {
        if (sfToolkit::isPathAbsolute($template)) {
            $this->decoratorDirectory = dirname((string) $template);
            $this->decoratorTemplate = basename((string) $template);
        } else {
            $this->decoratorTemplate = $template;
        }

        if (!strpos((string) $this->decoratorTemplate, '.')) {
    ...
    {
        if (sfToolkit::isPathAbsolute($template)) {
            $this->directory = dirname((string) $template);
            $this->template = basename((string) $template);
        } else {
