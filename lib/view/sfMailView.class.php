...
class sfMailView extends sfPHPView
{
    ...
    public function render($templateVars = null)
    {
        ...
        foreach ($all_templates as $templateFile) {
            if (preg_match('/\.([^.]+?)\.php$/', (string) $templateFile, $matches)) {
        ...
        $config_prefix = 'sf_mailer_'.strtolower((string) $this->moduleName).'_';

        $vars = ['mailer', 'priority', 'content_type', 'charset', 'encoding', 'wordwrap', 'hostname', 'port', 'domain', 'username', 'password'];
