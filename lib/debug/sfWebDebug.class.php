...
class sfWebDebug
{
    protected $log = [];
    protected $short_log = [];
    protected $max_priority = 1000;
    protected $types = [];
    protected $last_time_log = -1;
    ...
    protected function formatLogLine($log_line)
    {
        ...
        if (!$constants) {
            foreach (['sf_app_dir', 'sf_root_dir', 'sf_symfony_lib_dir', 'sf_symfony_data_dir'] as $constant) {
                $constants[realpath(sfConfig::get($constant)).DIRECTORY_SEPARATOR] = $constant.DIRECTORY_SEPARATOR;
            }
        }

        // escape HTML
        $log_line = htmlentities((string) $log_line, ENT_QUOTES, sfConfig::get('sf_charset'));

        // replace constants value with constant name
        $log_line = str_replace(array_keys($constants), array_values($constants), $log_line);

        $log_line = sfToolkit::pregtr($log_line, ['/&quot;(.+?)&quot;/s' => '"<span class="sfWebDebugLogInfo">\\1</span>"', '/^(.+?)\(\)\:/S' => '<span class="sfWebDebugLogInfo">\\1()</span>:', '/line (\d+)$/' => 'line <span class="sfWebDebugLogInfo">\\1</span>']);

        // special formatting for SQL lines
        $log_line = $log_line ? preg_replace('/\b(SELECT|FROM|AS|LIMIT|ASC|COUNT|DESC|WHERE|LEFT JOIN|INNER JOIN|RIGHT JOIN|ORDER BY|GROUP BY|IN|LIKE|DISTINCT|DELETE|INSERT|INTO|VALUES)\b/', '<span class="sfWebDebugLogInfo">\\1</span>', $log_line) : $log_line;

        // remove username/password from DSN
        if (str_contains($log_line, 'DSN')) {
            $log_line = $log_line ? preg_replace("/=&gt;\s+'?[^'\s,]+'?/", "=&gt; '****'", $log_line) : $log_line;
        }
    ...
    public function getResults()
    {
        ...
        <tr>
          ...
          <th>message</th>
            ...
            foreach ($this->log as $logEntry) {
                ...
                if (str_starts_with((string) ($type = $logEntry['type']), 'sf')) {
                    $type = substr((string) $type, 2);
                }
        ...
        if (sfConfig::get('sf_debug') && sfConfig::get('sf_cache')) {
            $self_url = $_SERVER['PHP_SELF'].((!str_contains((string) $_SERVER['PHP_SELF'], '_sf_ignore_cache')) ? '?_sf_ignore_cache=1' : '');
     ...
     * @param bool true if the content is new in the cache, false otherwise
     *
    ...
    public function decorateContentWithDebug($internalUri, $content, $new = false)
    {
        ...
        if (!sfConfig::get('sf_web_debug') || !$content || !str_contains((string) $context->getResponse()->getContentType(), 'html')) {
            return $content;
        }
      ...
      <div style="height: 16px; padding: 2px"><a href="#" onclick="sfWebDebugToggle(\'sub_main_info_'.$id.'\'); return false;"><strong>cache information</strong></a>&nbsp;<a href="#" onclick="sfWebDebugToggle(\'sub_main_'.$id.'\'); document.getElementById(\'main_'.$id.'\').style.border = \'none\'; return false;">'.image_tag(sfConfig::get('sf_web_debug_web_dir').'/images/close.png').'</a>&nbsp;</div>
        ...
        [uri]&nbsp;'.htmlentities((string) $internalUri, ENT_QUOTES, sfConfig::get('sf_charset')).'<br />
