...
class sfMessageSource_Creole extends sfMessageSource
{
    ...
    protected function &loadData($variant)
    {
        ...
        $rs = $stmt->executeQuery([$variant], ResultSet::FETCHMODE_NUM);
    ...
    protected function getLastModified($source)
    {
        ...
        $rs = $stmt->executeQuery([$source], ResultSet::FETCHMODE_NUM);
    ...
    protected function getCatalogueDetails($catalogue = 'messages')
    {
        ...
        $rs = $stmt->executeQuery([$name], ResultSet::FETCHMODE_NUM);
        ...
        $rs = $stmt->executeQuery([$cat_id], ResultSet::FETCHMODE_NUM);
    ...
    protected function updateCatalogueTime($cat_id, $variant)
    {
        ...
        $result = $stmt->executeUpdate([$time, $cat_id]);
    ...
    public function save($catalogue = 'messages')
    {
        ...
        try {
            ...
            foreach ($messages as $key => $message) {
                $rs = $stmt->executeQuery([$message], ResultSet::FETCHMODE_NUM);
                if ($rs->next()) {
                    unset($messages[$key]);
                }
            }
        ...
        try {
            ...
            foreach ($messages as $message) {
                $stmt->executeUpdate([$cat_id, $message, '', '', $time, $time]);
                ++$inserted;
            }
    ...
    public function delete($message, $catalogue = 'messages')
    {
        ...
        $rows = $stmt->executeUpdate([$cat_id, $message]);
    ...
    public function update($text, $target, $comments, $catalogue = 'messages')
    {
        ...
        $rows = $stmt->executeUpdate([$target, $comments, $time, $cat_id, $text]);
    ...
    public function catalogues()
    {
        ...
        $result = [];
        while ($rs->next()) {
            $details = explode('.', (string) $rs->getString(1));
            if (!isset($details[1])) {
                $details[1] = null;
            }
