...
class sfPropelData extends sfData
{
    ...
    public function loadDataFromArray($data)
    {
        ...
        foreach ($data as $class => $datas) {
            ...
            $column_names = call_user_func_array([$peer_class, 'getFieldNames'], [BasePeer::TYPE_FIELDNAME]);
    ...
    protected function doDeleteCurrentData($fixture_files)
    {
        ...
        foreach ($fixture_files as $fixture_file) {
            ...
            foreach (array_reverse($classes) as $class) {
                ...
                $peer_class = $class.'Peer';
                call_user_func([$peer_class, 'doDeleteAll'], $this->con);
    ...
    public function dumpData($directory_or_file = null, $tables = 'all', $connectionName = 'propel')
    {
        ...
        if ('all' === $tables || is_null($tables)) {
            ...
            foreach ($files as $file) {
                $mapBuilderClass = basename((string) $file, '.php');
        ...
        // load map classes
        array_walk($tables, [$this, 'loadMapBuilder']);
        ...
        foreach ($tables as $tableName) {
            ...
            foreach ($tableMap->getColumns() as $column) {
                $col = strtolower((string) $column->getColumnName());
        ...
        if ($sameFile) {
            ...
            foreach ($tables as $tableName) {
                ...
                file_put_contents(sprintf('%s/%03d-%s.yml', $directory_or_file, ++$i, $tableName), Spyc::YAMLDump([$tableName => $dumpData[$tableName]]));
            }
        }
    }
    ...
    protected function fixOrderingOfForeignKeyDataInSameTable($resultsSets, $tableName, $column, $in = null)
    {
        $rs = $this->con->executeQuery(sprintf('SELECT * FROM %s WHERE %s %s',
            ...
            strtolower((string) $column->getColumnName()),
            is_null($in) ? 'IS NULL' : 'IN ('.$in.')'
        ));
        $in = [];
        while ($rs->next()) {
            $in[] = "'".$rs->get(strtolower((string) $column->getRelatedColumnName()))."'";
        }
