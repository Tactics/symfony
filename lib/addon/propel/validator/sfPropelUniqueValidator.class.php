...
class sfPropelUniqueValidator extends sfValidator
{
    public function execute(&$value, &$error)
    {
        ...
        $columnName = call_user_func([$className, 'translateFieldName'], $this->getParameter('column'), BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_COLNAME);

        $c = new Criteria();
        $c->add($columnName, $value);
        $object = call_user_func([$className, 'doSelectOne'], $c);

        if ($object) {
            $tableMap = call_user_func([$className, 'getTableMap']);
            foreach ($tableMap->getColumns() as $column) {
                ...
                $primaryKey = call_user_func([$className, 'translateFieldName'], $column->getPhpName(), BasePeer::TYPE_PHPNAME, BasePeer::TYPE_FIELDNAME);
