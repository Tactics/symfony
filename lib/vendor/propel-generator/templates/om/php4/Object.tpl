...
<?php if (! $table->isAlias() && isset($addSaveMethod) && $addSaveMethod && ! $table->isReadOnly()) { ?>
  ...
  */
  function delete()
  {
    ...
    $e = <?php echo $table->getPhpName() ?>Peer::doDelete($this, Param::set($connection));
...
?>
  ...
  */
  function save()
  {
    ...
    $e = $this->doSave(Param::set($connection));
    if (Propel::isError($e)) { $connection->rollback(); return $e; }
...
<?php
  ...
  {
?>
  function doSave($connection = null)
<?php
  } else {
?>
  function save()
...
?>
    ...
    Propel::assertParam($connection, '<?php echo $table->getPhpName(); ?>', 'save', 1);
...
?>
      ...
      if ($this->isModified()) {
        if ($this->isNew()) {
          $pk = <?php echo $table->getPhpName() ?>Peer::doInsert($this, <?php if($complexObjectModel) {?>$connection<?php } else { ?>Param::set($connection)<?php } ?>);
...
?>
          ...
          $e = <?php echo $table->getPhpName() ?>Peer::doUpdate($this, <?php if($complexObjectModel) {?>$connection<?php } else { ?>Param::set($connection)<?php } ?>);
