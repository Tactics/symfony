...
class sfCompileConfigHandler extends sfYamlConfigHandler
{
    ...
    public function execute($configFiles)
    {
        ...
        foreach ($config as $file) {
            ...
            $contents = preg_replace_callback(['#(require|include)(_once)?\((sfConfigCache::getInstance\(\)|\$configCache)->checkConfig\([^_]+sf_app_config_dir_name[^\.]*\.\'/([^\']+)\'\)\);#m', '#()()(sfConfigCache::getInstance\(\)|\$configCache)->import\(.sf_app_config_dir_name\.\'/([^\']+)\'(, false)?\);#m'],
                [$this, 'insertConfigFileCallback'], $contents);

            // strip php tags
            $contents = sfToolkit::pregtr($contents, ['/^\s*<\?(php)?/m' => '', '/^\s*\?>/m' => '']);
