...
class sfValidatorConfigHandler extends sfYamlConfigHandler
{
    ...
    public function execute($configFiles)
    {
        ...
        foreach (['methods', 'names'] as $category) {
     ...
     *
     * @return bool Returns true if there is some validators for this file/parameter
     */
    protected function generateRegistration($method, &$data, &$methods, &$names, &$validators)
    {
        ...
        foreach ($methods[$method] as $name) {
            if (preg_match('/^([a-z0-9_-]+)\{([a-z0-9\s_-]+)\}$/i', (string) $name, $match)) {
    ...
    protected function loadNames(&$configFiles, &$method, &$methods, &$names, &$config, &$list)
    {
        ...
        foreach ($array as $name) {
            ...
            } elseif (str_contains((string) $name, '{') || str_contains((string) $name, '}')) {
    ...
    {
        $defaultMethods = $config['methods'] ?? ['post'];
        $config['methods'] = [];
        ...
        foreach ($config['names'] as $name => $values) {
            ...
            foreach ($values as $validator => $params) {
                if (in_array($validator, ['required', 'group', 'group_msg', 'parent', 'file', 'methods'])) {
                    continue;
                }

                // class or validator
                if (!isset($config[$validator])) {
                    $config[$validator] = ['class' => $validator];
                }
                ...
                if ($params) {
                    ...
                    $config[$validatorName]['param'] = array_merge(isset($config[$validator]['param']) ? (array) $config[$validator]['param'] : [], $params);
                }
