...
class sfConfigCache
{
    ...
    {
        $fileCache = new sfFileCache(dirname((string) $cache));
        $fileCache->initialize(['lifeTime' => 86400 * 365 * 10, 'automaticCleaningFactor' => 0]);
        $fileCache->setWriteControl(true);
        $fileCache->setSuffix('');

        if (!$fileCache->set(basename((string) $cache), '', $data)) {
            $fileCache->remove(basename((string) $cache), '');
