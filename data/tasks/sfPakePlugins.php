...
function run_plugin_list($task, $args)
{
  ...
  foreach ($installed as $channel => $packages)
  {
    foreach ($packages as $package)
    {
      ...
      pake_echo(sprintf(" %-40s %10s-%-6s %s", pakeColor::colorize($pobj->getPackage(), 'INFO', pake_STDOUT()), $pobj->getVersion(), $pobj->getState() ? $pobj->getState() : null, pakeColor::colorize(sprintf('# %s (%s)', $channel, $registry->getChannel($channel)->getAlias()), 'COMMENT', pake_STDOUT())));
    }
  }
}
